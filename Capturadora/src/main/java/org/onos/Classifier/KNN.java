...
public class KNN {
    public static String filename = "//root//onos//apache-karaf-4.2.8//csv//dataset_final.arff";
    public static IBk knn_classifier;
    private static Instances training_data;
    private static Instances test_data;
    private static int num_atributes;

    public KNN(Logger log) throws Exception {
        Instances all_data = get_training_data();
        log.info("Dividiendo dataset...");
    ...
    private void testEvaluation(Logger log) throws Exception {
        ...
        eval.evaluateModel(knn_classifier, test_data);
    ...
    public String predict(String parameters_string, Logger log) throws Exception {
        ...
        // label instances
        double prediction = knn_classifier.classifyInstance(new_input.lastInstance());

        // get the name of the class value
        String prediction_name = new_input.classAttribute().value((int) prediction);

        double dist[] = knn_classifier.distributionForInstance(new_input.lastInstance());
    ...
    private void knnCreation() throws Exception {
        knn_classifier = new IBk(15);
        // Train the model
        knn_classifier.buildClassifier(training_data);
    }
